...
{{
  ...
  "mse_on_admission": "Write the mental state on admission in concise clinical prose covering: appearance and behaviour; speech; mood (subjective and objective); thought form and content (include specific delusions or ideation if documented); perceptions (hallucinations — type and content); insight into illness and need for treatment. This establishes the baseline from which progress will be measured. 3-5 sentences. Important: reflect the actual acuity — if the patient was paranoid, distressed, or hallucinating on admission, this must be apparent. Do not describe engagement as normal if the patient's mental state was significantly disturbed. For example, if paranoid beliefs were active, engagement was likely variable and influenced by those beliefs — say so.",

  "clinical_narrative": "Write the clinical story of the admission in 4-5 sentences. This is the core explanatory section. Cover: the nature of the disorder — what kind of illness this is, its characteristic features, and what typically happens when untreated or when precipitants are present; the degree of the disorder on admission — how severe this episode was, what specific symptoms and behaviours it produced, how it compared to the patient's baseline, and include a brief explicit statement of nature and degree such as 'The nature of the disorder was characterised by [symptoms] and the degree was [mild/moderate/severe] on admission given [specific features]'; the working or confirmed diagnosis; and the initial treatment approach. Write as a consultant explaining a complex presentation to a GP — not as a checklist.",

  "progress_on_ward": "Write a temporal account of progress in 4-5 sentences. Do NOT use headings or labels such as 'EARLY IN ADMISSION:'. Instead write in continuous prose using natural temporal connectors: begin with 'Early in admission...' then transition with 'As admission progressed...' or 'Over the following days...' and close with 'In the later stages of admission...' or 'By the time of discharge...'. Cover: early mental state and behaviour; trajectory of improvement or deterioration; response to medications; engagement with the clinical team; any turning points or incidents; and the clinical picture that made discharge appropriate.",
  ...
  "diagnosis": "State the discharge diagnosis using ICD-10 terminology where possible. If diagnostic uncertainty remains (e.g. substance-induced vs primary psychotic disorder), state this clearly using language such as: 'A primary psychotic disorder cannot be excluded at this stage, although the current presentation is most consistent with a substance-induced aetiology.' Do not use awkward phrases like 'uncertainty due to chronicity'. One to three sentences only.",

  "legal_status_at_discharge": "State the patient's legal status at discharge in one sentence. If they were detained under the MHA, state the section and whether they were discharged from it, had it lifted, or were discharged conditionally. If informal, state this. Example: 'He was discharged from Section 2 of the Mental Health Act following clinical improvement.' If not documented, state this clearly.",

  "risk_and_crisis_plan": "3-4 sentences. State the current risk level at discharge to self and others. Include conditional risk where relevant — specifically, if risk is contingent on resumption of substance use, disengagement from follow-up, or medication non-adherence, state this using calibrated discharge phrasing (e.g. 'There remains a risk of deterioration, particularly if substance use resumes' or 'Risk of relapse would be increased should the patient disengage from follow-up'). Do not use strong or alarming phrasing such as 'would increase significantly'. State the specific warning signs the GP and patient should watch for. State the crisis plan: what the patient should do if they deteriorate, who to contact, and what emergency options exist.",

  "discharge_medications": "Write the discharge medications as natural clinical prose — not as a structured list or dictionary. Describe in sentences: which medications were continued from before admission; which were newly started during this admission (with dose, route, frequency, and brief reason); any medications that were offered but refused by the patient (with reason if documented); and any PRN medications at discharge with indication. Use natural connectors: 'He was continued on...', 'During this admission he was started on...', 'Antipsychotic medication was offered but declined...', 'A PRN of... was prescribed for...'. If a category does not apply, omit it. Do not use bullet points, labels, or dictionary-style formatting. Also ensure consistent use of singular or plural when referring to third parties (e.g. use 'his neighbour' consistently, not alternating between 'neighbour' and 'neighbours').",

  "follow_up_and_gp_actions": "3-4 sentences. State the follow-up arrangements: who is following up (CMHT, EIT, outpatient clinic), approximate timeframe, and any referrals made during admission. Then state clearly and specifically what action is required from the GP. Use precise language — not 'the GP may wish to consider' but 'the GP is requested to [action]'. Include medication monitoring requirements, blood tests (e.g. metabolic monitoring for antipsychotics, lithium levels), physical health review, DVLA notification if relevant, or safeguarding actions. If the GP has no specific actions required, state this explicitly.",
...
def render_discharge(ds_data, patient_name, show_debug):
    ...
    fields = [
        ...
        ("Legal status at discharge",       "legal_status_at_discharge"),
        ("Diagnosis",                       "diagnosis"),
...
{{
  ...
  "q24_recommendations": "Write Q24 in TWO PARTS. PART ONE — CLOSING ARGUMENT (mandatory): Write ONE continuous paragraph. No headings. No sub-points. No phrases such as 'in my clinical opinion'. Do not restate the diagnosis generically. The paragraph must flow in this exact order: (1) current clinical state briefly; (2) nature of illness — the pattern in THIS patient; (3) degree — current severity and functional impact; (4) what will likely happen if discharged, with a specific timeframe; (5) why community management will fail; (6) the final conclusion on detention. Use assertive, clinically reasoned language. The argument must build logically and feel inevitable. TARGET STYLE: 'Zeyad continues to present with a relapsing psychotic illness characterised by persistent lack of insight and ongoing refusal of medication. The current degree remains clinically significant, with impaired judgement and limited ability to engage meaningfully with treatment. If discharged at this stage, it is highly likely that he would discontinue treatment, leading to deterioration of his mental state within weeks, with increasing self-neglect and risk associated with relapse. Community management is not currently sufficient due to lack of insight, poor adherence, and inability to reliably mitigate risk outside a structured setting. For these reasons, continued detention remains necessary for treatment and risk management.' FAILURE CONDITIONS — output is incorrect if: it uses bullet points; it says 'criteria are met' without explanation; it repeats earlier sections instead of integrating them; it lacks a predicted outcome with timeframe; it reads as a summary rather than a conclusion. PART TWO — NUMBERED PLAN: After the closing argument paragraph, write a numbered plan: 1. Decision on legal status; 2. Medication plan; 3. Monitoring and observations; 4. MDT input and next steps; 5. Discharge planning considerations if appropriate.",
