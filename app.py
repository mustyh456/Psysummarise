...
{{
  ...
  "risk_and_crisis_plan": "3-4 sentences. State the current risk level at discharge to self and others. Include conditional risk where relevant — specifically, if risk is contingent on resumption of substance use, disengagement from follow-up, or medication non-adherence, state this explicitly (e.g. 'Risk of deterioration and self-harm would increase significantly if substance use were to resume'). State the specific warning signs the GP and patient should watch for. State the crisis plan: what the patient should do if they deteriorate, who to contact, and what emergency options exist.",

  "discharge_medications": "Write the medication list in four clearly labelled parts. CONTINUED: medications that were already prescribed before admission and are continuing unchanged. NEWLY STARTED: medications initiated during this admission, with dose, route, frequency, and brief reason for starting. REFUSED: any medications offered but declined by the patient, with brief reason if documented. PRN: any PRN medications prescribed at discharge with indication and maximum frequency if documented. If a section is not applicable, write 'None'. Format each medication as: Drug name dose route frequency — e.g. Mirtazapine 30mg oral nocte.",
